/**
 * Browser-side mirror of the control-plane CSV/TSV rules. Keeping the grid
 * and the acceptance endpoint on the same small contract makes previews,
 * selections, and stale checks predictable.
 */
...
export type TableResult<T> = T | { error: TableError; message: string; address?: string };
...
export function parseDelimitedTable(content: string, path: string): TableResult<ParsedDelimitedTable> {
  ...
  return { rows, delimiter, lineEnding, hasFinalLineEnding: lastRecordEnded, bom };
}
...
export function applyDelimitedEdits(content: string, path: string, edits: TableEdit[]): TableResult<{ content: string; table: ParsedDelimitedTable }> {
  if (!Array.isArray(edits) || edits.length === 0) return { error: "invalid-address", message: "At least one table cell is required." };
  if (edits.length > TABLE_EDIT_CAP) return { error: "out-of-range", message: `A table proposal can contain at most ${TABLE_EDIT_CAP} cells.` };
  ...
  for (const edit of edits) {
    ...
    seen.add(address);
    if (position.row >= TABLE_ROW_CAP || position.col >= TABLE_COL_CAP) return { error: "out-of-range", message: `Cell ${address} is outside the first ${TABLE_ROW_CAP} rows and ${TABLE_COL_CAP} columns.`, address };
    if (position.row >= rows.length || position.col >= maxColumns) return { error: "out-of-range", message: `Cell ${address} is outside the current table.`, address };
    while (rows[position.row]!.length <= position.col) rows[position.row]!.push("");
    if (rows[position.row]![position.col] !== edit.before) return { error: "stale", message: `Cell ${address} no longer contains the expected value.`, address };
